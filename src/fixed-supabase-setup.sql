...
-- First, create the users table (must be first due to foreign key dependencies)
...
-- Create auth_links table (without foreign key constraint initially)
...
BEGIN
  ...
  -- Add foreign key for auth_links.user_pin if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'auth_links_user_pin_fkey' 
    AND table_name = 'auth_links'
  ) THEN
    ALTER TABLE auth_links ADD CONSTRAINT auth_links_user_pin_fkey 
    FOREIGN KEY (user_pin) REFERENCES users(pin) ON DELETE CASCADE;
  END IF;

  -- Add foreign key for auth_links.auth_id if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'auth_links_auth_id_fkey' 
    AND table_name = 'auth_links'
  ) THEN
    ALTER TABLE auth_links ADD CONSTRAINT auth_links_auth_id_fkey 
